...
class MomentumExecutorService:
    ...
    def run_once(self, *, force: bool = False) -> dict[str, Any]:
        ...
        if action == "WAIT":
            synced = self._sync_local_position(local_position, decision=decision, reason="momentum_wait")
            return {"action": "WAIT", "decision": decision, "local_position": synced}
        if action == "HOLD":
            synced = self._sync_local_position(local_position, decision=decision, reason="momentum_hold")
            return {"action": "HOLD", "decision": decision, "local_position": synced}
        if action == "SELL":
    ...
    def _response_payload(self, response: requests.Response) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Remote momentum endpoint did not return JSON. status={response.status_code} "
                f"content_type={content_type} body={body!r}"
            ) from exc
    ...
    def _normalize_decision(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        payload.setdefault("top_watch_asset", None)
    ...
    def _sync_local_position(self, position, *, decision: dict[str, Any], reason: str) -> dict[str, Any] | None:
        if not position:
            return None
        try:
            mark_price = self.binance.current_price(position.symbol)
            pnl = (mark_price - float(position.entry_price or mark_price)) * float(position.quantity or 0.0)
            position.mark_price = mark_price
            position.unrealized_pnl = pnl
            position.meta = {
                **(position.meta or {}),
                "strategy": "momentum_rotation",
                "last_momentum_decision_action": decision.get("action"),
                "last_momentum_decision_reason": decision.get("reason"),
                "last_momentum_decision_at": datetime.now(timezone.utc).isoformat(),
                "last_momentum_sync_reason": reason,
            }
            self.db.commit()
            self.db.refresh(position)
        except Exception as exc:
            self.db.rollback()
            return {**(self._position_payload(position) or {}), "sync_error": str(exc)}
        return self._position_payload(position)
    ...
    def _sell_local_position(self, position, *, mode: str, reason: str) -> dict[str, Any]:
        ...
        order = self.orders.create_order(
            ...
            filled_price=avg_price,
            status="filled",
            meta={"mode": mode_meta, "strategy": "momentum_rotation", "reason": reason, "exchange": exchange},
        )
