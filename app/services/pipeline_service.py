...
class PipelineService:
    ...
    def _lookback_key(self, interval: str) -> str:
        return f"kraken_lookback_{interval}"

    def _bundle_limits(self, execution_interval: str) -> dict[str, int]:
        runtime = self.collector.runtime.get("market_data", self.collector.runtime["kraken"])
        return {
            execution_interval: int(runtime.get(self._lookback_key(execution_interval), 180)),
            "1h": int(runtime["kraken_lookback_1h"]),
            "4h": int(runtime["kraken_lookback_4h"]),
        }
    ...
    def run_once(self, limit: int | None = None) -> dict:
        ...
        max_workers = max(1, int(self.collector.runtime.get("market_data", self.collector.runtime["kraken"]).get("kraken_collect_max_workers", 4)))
