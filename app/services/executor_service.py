...
class ExecutorService:
    ...
    def _execute_live_candidate(self, candidate, quantity: float) -> dict:
        ...
        normalized = self.binance.normalize_order(candidate.symbol, quantity=quantity, target_price=candidate.target_price, stop_price=None)
        ...
        tp_resp = self.binance.place_limit_sell(
            candidate.symbol,
            quantity=filled_qty,
            price=normalized['target_price'],
        )
        tp_local = self.orders.create_order(
            ...
            meta={'mode': 'live', 'exchange': tp_resp, 'exchange_order_id': tp_resp.get('orderId')},
        )
        position.meta = {
            **(position.meta or {}),
