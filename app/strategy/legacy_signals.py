...
def _trade_count_ok(candles: list[dict[str, Any]], index: int) -> bool:
    ...
    Exchange quote volume can occasionally look usable on thin books while the
    ...
    if avg_trades <= 0:
        ...
        # Do not block them forever; new executor-pushed candles will populate this.
        return True
...
def volumes(candles: list[dict[str, Any]]) -> list[float]:
    ...
    Prefer quote asset volume because it is normalized in the quote
