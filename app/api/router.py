...
from app.api.routes import admin_market_data, admin_settings, assets, execution, executor, fills, health, live_runs, market_data, momentum, momentum_backtest, momentum_engine, orders, pipeline, positions, services, trade_candidates
...
api_router.include_router(execution.router, prefix='/api/v1', tags=['kraken-execution'])
api_router.include_router(admin_settings.router, prefix='/api/v1', tags=['admin-settings'])
api_router.include_router(admin_market_data.router, prefix='', tags=['admin-market-data'])
...
api_router.include_router(momentum_backtest.router, prefix='/api/v1/momentum-backtest', tags=['momentum-backtest'])
