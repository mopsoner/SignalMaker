...
if [ "${RUN_STANDALONE_FRONTEND:-0}" != "1" ]; then
  echo "Standalone frontend server disabled: signalmaker-api serves frontend/dist on APP_PORT=${EXECUTOR_API_PORT:-${APP_PORT:-8080}}."
  echo "Open http://127.0.0.1:${EXECUTOR_API_PORT:-${APP_PORT:-8080}}/admin.html"
