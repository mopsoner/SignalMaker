...
def run_once() -> dict:
    with SessionLocal() as db:
        ...
        return {"decision": decision, "execution_result": MomentumLiveExecutionService(db).execute_decision(decision)}


def assert_live_configuration() -> None:
    problems = []
    if not settings.momentum_live_enabled:
        problems.append("MOMENTUM_LIVE_ENABLED must be true")
    ...
    if settings.momentum_live_mode == "margin" and not settings.kraken_margin_execution_enabled:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if once:
            break
        time.sleep(settings.momentum_live_interval_seconds)
