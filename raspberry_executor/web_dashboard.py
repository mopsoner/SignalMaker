...
def event_message(event):
    ...
    if "insufficient balance" in low:
        return "Insufficient balance. Binance refused the order because available balance was too low or reserved."
    if "invalid_oco_price_order" in low:
        return "OCO rejected before submit: target/current/stop order is no longer valid."
