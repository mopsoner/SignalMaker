...
The app keeps a local SQLite state database to avoid duplicate execution after restart. The Raspberry reads `/api/v1/momentum` rankings from `main` and creates BUY/HOLD/ROTATE decisions locally, unless `MOMENTUM_DECISION_PATH` is explicitly set to a custom decision endpoint.
