...
class KrakenMarginClient:
    ...
    def margin_order(self, symbol: str, side: str, quantity: str, order_type: str = "MARKET", *, price: str | None = None, time_in_force: str | None = None) -> dict:
        ...
        if self.dry_run:
            ...
            return {"orderId": f"{order_prefix}-{int(time.time())}", "status": status, "executedQty": str(quantity) if type_lower == "market" else "0", "dry_run": True, "exchange": "kraken", "symbol": symbol.upper(), "side": side.upper(), "type": order_type, "quantity": quantity, "price": price, "leverage": leverage}
        params: dict[str, Any] = {"pair": self.kraken._pair_key(symbol), "type": side_lower, "ordertype": type_lower, "volume": quantity, "leverage": leverage}
        ...
        return {"orderId": txid, "txid": result.get("txid"), "status": "NEW", "exchange": "kraken", "symbol": symbol.upper(), "side": side.upper(), "type": order_type, "quantity": quantity, "price": price, "leverage": leverage, "result": result}
