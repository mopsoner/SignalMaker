...
def api_request(path: str, params: dict[str, Any] | None = None, *, method: str = "GET") -> Any:
    ...
    req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": "SignalMaker-Raspberry-TUI"}, method=method)
    try:
...
def as_rows(payload: Any) -> list[Any]:
    ...
    if isinstance(payload, dict):
        for key in ("items", "rows", "data", "results", "services", "workers"):
