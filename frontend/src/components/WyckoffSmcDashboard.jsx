...
const alignmentStatus = (row) => trigger(row)?.alignment_status || confirmationModel(row)?.fifteen_min_alignment || gate(row)?.execution_15m_alignment || 'unknown'
const alignmentLabel = (row) => {
  const status = alignmentStatus(row)
  if (status === 'aligned') return '15m aligned'
  if (status === 'neutral_not_opposed') return '15m not opposed'
  if (status === 'opposed') return '15m opposed'
  return '15m alignment check'
}

const confirmationLabel = (row) => {
  const model = confirmationModel(row)
  const status = alignmentStatus(row)
  if (model.entry_mode === '15m_confirmed') return `15m aligned${model.confirmed_by_1h ? ' + 1H' : ''}`
  if (model.entry_mode === '1h_setup_15m_alignment_required') return oneHourDecision(row)?.valid || model.confirmed_by_1h ? `1H setup valid · ${alignmentLabel(row)}` : 'waiting 1H setup'
  if (model.confirmed_by_1h || oneHourDecision(row)?.valid) return `1H setup valid · ${alignmentLabel(row)}`
  if (model.confirmed_by_15m || trigger(row)?.valid) return alignmentLabel(row)
  if (stage(row) === 'waiting_1h_event' || blockedAt(row) === 'decision_1h') return 'waiting 1H setup'
  if (stage(row) === 'awaiting_15m_alignment' || blockedAt(row) === '15m_alignment' || status === 'opposed') return 'waiting 15m alignment'
  return model.entry_mode || 'wait'
}

const oneHourConfirmed = (row) => Boolean(confirmationModel(row)?.confirmed_by_1h || oneHourDecision(row)?.valid || wyckoffStatus(row) === 'execution_ready')
const fifteenMinAligned = (row) => alignmentStatus(row) === 'aligned'
const fifteenMinNotOpposed = (row) => ['aligned', 'neutral_not_opposed'].includes(alignmentStatus(row))
const fifteenMinOpposed = (row) => alignmentStatus(row) === 'opposed'
...
const waitingFifteenMinAlignment = (row) => !tradeCandidate(row) && !tradeReady(row) && (stage(row) === 'awaiting_15m_alignment' || blockedAt(row) === '15m_alignment' || String(plannerReason(row)).includes('15m_alignment'))
...
const executionReady = (row) => tradeReady(row) || tradeCandidate(row) || stage(row) === 'confirm' || wyckoffStatus(row) === 'execution_ready' || (oneHourConfirmed(row) && fifteenMinNotOpposed(row))
...
const actionableWatch = (row) => tradeCandidate(row) || tradeReady(row) || (oneHourConfirmed(row) && fifteenMinNotOpposed(row)) || ['confirm', 'confirm_watch'].includes(stage(row))
...
export function DecisionPath({ row }) {
  ...
  return <div style={{ display: 'grid', gap: 4, minWidth: 280 }}>
    ...
    <div style={{ fontSize: 11, opacity: 0.85 }}>{currentStage} · {detail || 'wait'} · {strongZone(row) ? 'zone ok' : 'diagnostic zone'} · {alignmentLabel(row)}</div>
  </div>
}

export function MobileAssetCards({ rows, detailUrl, tradingViewLink, symbolFor = (row) => row.symbol, assetMeta }) {
...
export function MarketFilterChips({ filters, marketFilter, setMarketFilter }) {
  return <div className="filter-chips" role="tablist" aria-label="Market filters">{filters.map(([key, label]) => <button key={key} type="button" className={`filter-chip ${marketFilter === key ? 'active' : ''}`} onClick={() => setMarketFilter(key)}>{label}</button>)}</div>
}

export default function WyckoffSmcDashboard({
  loadAssets,
  detailUrl,
  tradingViewLink,
  symbolFor = (row) => row.symbol,
  assetMeta,
  title = 'Dashboard 360',
  subtitle = 'Market overview: 4H context/target, 1H Wyckoff-SMC setup, 15m alignment filter.',
  labels = {},
  marketFilters = [],
  pollingInterval = 15000,
}) {
  const [marketFilter, setMarketFilter] = useState('actionable')
  const [marketSelections, setMarketSelections] = useState(() => Object.fromEntries(marketFilters.map((filter) => [filter.key, ''])))
  const fetcher = useCallback(() => loadAssets(marketSelections), [loadAssets, marketSelections])
  const { data: assets = [], loading, error } = usePollingQuery(fetcher, pollingInterval)
  ...
  const counts = useMemo(() => ({
    ...
    fifteenMinAligned: assets.filter(fifteenMinAligned).length,
    fifteenMinNotOpposed: assets.filter(fifteenMinNotOpposed).length,
    fifteenMinOpposed: assets.filter(fifteenMinOpposed).length,
    ...
    waitingFifteenMinAlignment: assets.filter(waitingFifteenMinAlignment).length,
  ...
  const filteredAssets = useMemo(() => {
    ...
    if (marketFilter === 'fifteen_min_aligned') return assets.filter(fifteenMinAligned)
    if (marketFilter === 'fifteen_min_not_opposed') return assets.filter(fifteenMinNotOpposed)
    if (marketFilter === 'fifteen_min_opposed') return assets.filter(fifteenMinOpposed)
    ...
    if (marketFilter === 'waiting_15m_alignment') return assets.filter(waitingFifteenMinAlignment)
  ...
  return <div className="page-stack">
    ...
    {marketFilters.length ? <div className="page-actions" style={{ flexWrap: 'wrap' }}>{marketFilters.map((filter) => <label key={filter.key}>{filter.label}<select aria-label={filter.label} value={marketSelections[filter.key]} onChange={(event) => setMarketSelections((current) => ({ ...current, [filter.key]: event.target.value }))}><option value="">{filter.allLabel}</option>{filter.options.map((option) => <option key={typeof option === 'string' ? option : option.value} value={typeof option === 'string' ? option : option.value}>{typeof option === 'string' ? option : option.label}</option>)}</select></label>)}</div> : null}
    ...
    <details className="panel collapsible-panel" open><summary><h2>Market view 360</h2><span className="collapse-indicator">⌄</span></summary><div className="market-toolbar"><MarketFilterChips filters={filters} marketFilter={marketFilter} setMarketFilter={setMarketFilter} /><div className="market-toolbar-hint">Showing {sortedFilteredAssets.length} / {assets.length}</div></div><div className="desktop-market-table"><FoldableTable title="Assets" columns={columns} rows={sortedFilteredAssets} empty="No asset state available" defaultSortKey="score" defaultSortDir="desc" /></div><MobileAssetCards rows={sortedFilteredAssets} detailUrl={detailUrl} tradingViewLink={tradingViewLink} symbolFor={symbolFor} assetMeta={assetMeta} /></details>
  </div>
}
