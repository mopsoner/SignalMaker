...
const executionReady = (row) => row.stage === 'trade' || row.stage === 'confirm' || wyckoffStatus(row) === 'execution_ready' || Boolean(trigger(row)?.valid)
...
const blocked = (row) => wyckoffStatus(row) === 'blocked' || String(reason(row)).includes('blocked')
...
export default function DashboardPage() {
  ...
  const counts = useMemo(() => ({
    ...
    liquidity: assets.filter((x) => x.stage === 'liquidity').length,
  ...
  const filteredAssets = useMemo(() => {
    ...
    if (marketFilter === 'liquidity') return assets.filter((x) => x.stage === 'liquidity')
  ...
  const filters = [['all', `All (${assets.length})`], ['execution_ready', `Execution ready (${counts.ready})`], ['trade', `Trade (${counts.trade})`], ['confirm', `Confirm (${counts.confirm})`], ['zone', `Zone (${counts.zone})`], ['liquidity', `Liquidity (${counts.liquidity})`], ['bull', `Bull (${counts.bull})`], ['bear', `Bear (${counts.bear})`], ['swept', `Swept (${counts.swept})`], ['waiting_sweep', `Waiting sweep (${counts.waiting})`], ['strong_zone', `Strong zone (${counts.strongZones})`], ['with_target', `With target (${counts.withTarget})`], ['mss', `MSS (${counts.mss})`], ['bos', `BOS (${counts.bos})`], ['blocked', `Blocked (${counts.blocked})`]]

  return <div className="page-stack"><PageHeader title="Dashboard 360" subtitle="Market overview with debug links, projected targets, and asset drill-down." /><div className="stats-grid"><StatCard label="Tracked assets" value={assets.length} hint={`latest updated · limit ${assetLimit}`} /><StatCard label="Trade stage" value={counts.trade} /><StatCard label="Confirm stage" value={counts.confirm} /><StatCard label="Zone stage" value={counts.zone} /></div><div className="stats-grid"><StatCard label="Average score" value={avgScore} /><StatCard label="Strong zones" value={counts.strongZones} /><StatCard label="Bull / Bear" value={`${counts.bull} / ${counts.bear}`} /><StatCard label="Execution ready" value={counts.ready} /></div>{loading ? <div className="panel">Loading assets…</div> : null}{error ? <div className="panel error">{error}</div> : null}<FoldableTable title="Highest score assets" columns={columns.slice(0, 8)} rows={strongestAssets} empty="No asset state available" /><details className="panel collapsible-panel" open><summary><h2>Market view 360</h2><span className="collapse-indicator">⌄</span></summary><div className="market-toolbar"><div className="filter-chips" role="tablist" aria-label="Market filters">{filters.map(([key, label]) => <button key={key} type="button" className={`filter-chip ${marketFilter === key ? 'active' : ''}`} onClick={() => setMarketFilter(key)}>{label}</button>)}</div><div className="market-toolbar-hint">Showing {filteredAssets.length} / {assets.length}</div></div><div className="desktop-market-table"><FoldableTable title="Assets" columns={columns} rows={filteredAssets} empty="No asset state available" defaultSortKey="updated_at" defaultSortDir="desc" /></div><MobileAssetCards rows={filteredAssets} /></details></div>
}
