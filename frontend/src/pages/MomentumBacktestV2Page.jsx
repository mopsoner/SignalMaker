...
const bt = {
  ...
  sweep: () => req('/api/v1/momentum-backtest/runs/rsi-sweep', { method: 'POST', body: JSON.stringify({ ranges: [{ min: 45, max: 55 }, { min: 48, max: 60 }, { min: 50, max: 62 }, { min: 55, max: 65 }], base_settings: p3() }) }),
  compare: (ids) => req('/api/v1/momentum-backtest/compare?limit=800&run_ids=' + ids.join(',')),
}
function p3() {
  return { name: 'P3 RSI 1h 50-62 + 4h positive + 5m exit', entry_rsi_min: 50, entry_rsi_max: 62, entry_pool_top_n: 10, require_momentum_4h_positive: true, require_entry_structure_15m_valid: true, exit_structure_interval: '5m', volume_score_enabled: true }
}
...
export default function MomentumBacktestV2Page() {
  ...
  return <div className="page-stack"><PageHeader title="Momentum Backtesting V2" subtitle="Configure runs, sweep RSI ranges, and compare equity curves." />
    ...
    <section className="panel"><h2>Controls</h2><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button className="button" disabled={busy || workers?.momentum_backtest?.running} onClick={() => action(() => api.startWorker('momentum_backtest').then(refreshWorkers), 'Worker started')}>Start worker</button><button className="button" disabled={busy} onClick={() => action(() => bt.create(p3()), 'P3 queued')}>Run P3</button><button className="button" disabled={busy} onClick={() => action(() => bt.sweep(), 'RSI sweep queued')}>Run RSI sweep</button><button className="button" disabled={busy} onClick={() => { refresh(); refreshCurves(); refreshWorkers() }}>Refresh</button>{msg ? <span style={{ color: 'var(--muted)' }}>{msg}</span> : null}</div><p style={{ color: 'var(--muted)', fontSize: 13 }}>P3 = RSI 1h 50-62 + momentum 4h positif + structure 15m valide + sortie structure 5m + top 10 + volume dans le scoring.</p></section>
    <section className="panel two-col"><div><h2>Custom config</h2><textarea value={settings} onChange={(e) => setSettings(e.target.value)} style={{ width: '100%', minHeight: 200, background: 'rgba(255,255,255,.03)', color: 'var(--text)', border: '1px solid var(--line)', borderRadius: 12, padding: 12 }} /><button className="button" style={{ marginTop: 10 }} disabled={busy} onClick={() => action(() => bt.create(JSON.parse(settings)), 'Custom backtest queued')}>Run custom</button></div><div><h2>What changed</h2><p>Configurable RSI ranges, comparable curves, 5m structure exit, top 10 pool, 4h momentum filter and volume score.</p></div></section>
